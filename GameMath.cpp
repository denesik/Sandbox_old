...
namespace gm
{
	...
	::Json::Value gm::Point::Serialize() const
	{
		Json::Value val(Json::arrayValue);
		val.append(Json::Value(x));
		val.append(Json::Value(y));
		return val;
	}

	void gm::Point::UnSerialize( const ::Json::Value &val )
	{
		x = val[0U].asInt();
		y = val[1U].asInt();
	}
	...
	::Json::Value Size::Serialize() const
	{
		Json::Value val(Json::arrayValue);
		val.append(Json::Value(width));
		val.append(Json::Value(height));
		return val;
	}

	void Size::UnSerialize( const ::Json::Value &val )
	{
		width = val[0U].asInt();
		height = val[1U].asInt();
	}

	void Size::Inflate( const Size &s )
	{
		width += s.width;
		height += s.height;
	}

	void Size::Inflate( int w, int h )
	{
		width += w;
		height += h;
	}
	...
	::Json::Value Rectangle::Serialize() const
	{
		...
		val.append(Json::Value(x));
		val.append(Json::Value(y));
		val.append(Json::Value(w));
		val.append(Json::Value(h));
		return val;
	}

	void Rectangle::UnSerialize( const ::Json::Value &val )
	{
		x = val[0U].asInt();
		y = val[1U].asInt();
		w = val[2U].asInt();
